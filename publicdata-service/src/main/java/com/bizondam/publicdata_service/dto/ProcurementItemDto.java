package com.bizondam.publicdata_service.dto;

import lombok.Data;

@Data
public class ProcurementItemDto {
    private String prcrmntDivNm;
    private String cntrctDivNm;
    private String cntrctDlvrDivNm;
    private String cntrctDlvrReqDate;
    private String cntrctDlvrReqNo;
    private String cntrctDlvrReqChgOrd;
    private String fnlCntrctDlvrReqChgOrdYn;
    private String dminsttNm;
    private String dmndInsttDivNm;
    private String dminsttRgnNm;
    private String dminsttCd;
    private String prdctClsfcNo;
    private String prdctClsfcNoNm;
    private String dtilPrdctClsfcNo;
    private String dtilPrdctClsfcNoNm;
    private String prdctIdntNo;
    private String prdctIdntNoNm;
    private String prdctUprc;
    private String prdctQty;
    private String prdctUnit;
    private String prdctAmt;
    private String corpNm;
    private String corpEntrprsDivNmNm;
    private String cntrctDlvrReqNm;
    private String exclcProdctYn;
    private String cnstwkMtrlDrctPurchsObjYn;
    private String masYn;
    private String masCntrct2StepYn;
    private String uprcCntrctNo;
    private String uprcCntrctCngOrd;
    private String IntlCntrctDlvrReqDate;
    private String cntrctMthdNm;
    private String incdecQty;
    private String incdecAmt;
    private String dlvrPlceNm;
    private String dlvrTmlmtDate;
    private String bizno;
    private String dlvryCndtnNm;
}
